...
const CARDS = (total: number, receita: number, concluidos: number) => [
  {
    label: 'Agendamentos',
  ...
  },
  {
    ...
    value: total > 0 ? `${Math.round((concluidos / total) * 100)}%` : '—',
...
export function StatCards({ totalAgendamentos, receitaPrevista, concluidos }: StatCardsProps) {
  const cards = CARDS(totalAgendamentos, receitaPrevista, concluidos)
